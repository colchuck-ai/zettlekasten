...
Guidance for AI agents (and humans) working in this slip-box folder.

**Stand-alone:** Conventions and procedures below are complete for this directory.

**Nested in a larger repo:** If parent directories define **AGENTS.md**, read and apply every **AGENTS.md** from the repository root down through this folder (overlay: root first, then each level toward here; more specific wins on conflict).

**Full method (humans and agents):** Principles, anatomy, linking, structure, writing—**[zettelkasten.md](zettelkasten.md)**.

______________________________________________________________________

## Zettelkasten Conventions (Quick Reference)

- **File names**: date+slug in kebab-case (e.g. `2025-03-13-principle-of-atomicity.md`). Use `YYYY-MM-DD` then a slug.
- **Links**: Standard markdown `[text](path/to/note.md)`. Do not use wiki-style `[[links]]`.
- **Permanent notes**: In this folder’s root (not `inbox/`). When creating a new note, **name the file** date+slug; use [template-zettel.md](template-zettel.md) for the **content** (title, tags, Summary, Notes, Links). Include a creation-date tag with a hash: **#YYYY/MM/DD** (e.g. `#2025/03/13`) so systems with hierarchical tags can filter by year, year-month, or full date.
- **Inbox**: Use `inbox/` for fleeting captures; process into permanent notes here and clear the inbox regularly.
- **Source lines (provenance)**: When a note records where content came from, put it in italics as `*Source: …*` and **always include a markdown link** to the capturing artifact (daily entry, meeting note, chat log, ticket, etc.). Multiple sources → multiple links in one line. Avoid prose-only dates with no link.
...
4. Place the file in this folder’s root (not in inbox).
...
2. For each item: either create a new permanent note following **Create a permanent note** above (and add links with context), merge into an existing note, or discard. When promoting to permanent, **preserve or add hyperlinked** `*Source:*` lines (link to origin artifact).
